...
Aligner::Aligner(const Config& config) : config_(config){};//列表初始化
...
double Aligner::LidarOdomMinimizer(const std::vector<double>& x,
  ...
  //  struct OptData {
        ...
        //    bool time_cal;
        //  };
  OptData* d = static_cast<OptData*>(f_data);//强制转换数据类型
  //setOdomOdomTransforms():
  if (x.size() > 6) {
...
void Aligner::optimize(const std::vector<double>& lb,
  ...
  //x长度为:3和6
  nlopt::opt opt;
  //确定 local 和 global,先全局(global)后局部(local)
  //global:GN_DIRECT_L
  //global:LN_BOBYQA
  if (config_.local) {
  ...
  //opt_data:lidar and IMU data
  //LidarOdomMinimizer:构造需要优化的目标函数???
...
void Aligner::lidarOdomTransform(Lidar* lidar, Odom* odom) {
  ...
  opt_data.time_cal = config_.time_cal; //默认为true,有时间补偿

  size_t num_params = 6;
  if (config_.time_cal) {
    ++num_params;
  }
  //七行还是六行一列???七行,最后一行是时间戳
  std::vector<double> x(num_params, 0.0);
  //local默认为false,实际上进行全局优化
  if (!config_.local) {
    ...
    // 重点啊,哥哥opt_data和global_x???
    // opt_data:包含lidar,odom等信息
    // global_x:3*1数组??
    // 这个函数将迭代结束才会进入下一步
    optimize(lb, ub, &opt_data, &global_x);
    config_.local = true;//重置
